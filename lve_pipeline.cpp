...
namespace lve{
 ...
 const PipelineConfiguInfo& configInfo): lveDevice{device}{
 ...
 LvePipeline::~LvePipeline(){
    ...
    vkDestroyPipeline(lveDevice.device(), graphicsPipeline, nullptr);
     ...
     std::vector<char> LvePipeline::readFile(const std::string& filepath){
         ...
         if(!file.is_open()){
             throw std::runtime_error("failed to opne file: " + filepath);
         }
     ...
     void LvePipeline::createGraphicsPipeline(const std::string& vertFilepath, const std::string& fragFilepath,  const PipelineConfiguInfo& configInfo){
   
         assert(
             configInfo.pipelineLayout != VK_NULL_HANDLE && 
             "Cannot create grahpics pieline: no pipelineLayout provided in configInfo");
        ...
        pipelineInfo.stageCount = 2; //for now only two
     ...
     PipelineConfiguInfo LvePipeline::defaultPipelineConfigInfo(uint32_t width, uint32_t height){
        ...
        configInfo.rasterizationInfo.depthClampEnable = VK_FALSE; //z value must been between 0 and 1
        configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE; 
        configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL; //we draw entire triangle
        configInfo.rasterizationInfo.lineWidth = 1.0f;
        configInfo.rasterizationInfo.cullMode = VK_CULL_MODE_NONE;
        configInfo.rasterizationInfo.frontFace = VK_FRONT_FACE_CLOCKWISE; //witch side of triangle we see
        configInfo.rasterizationInfo.depthBiasEnable = VK_FALSE;
        configInfo.rasterizationInfo.depthBiasConstantFactor = 0.0f;  // Optional
        configInfo.rasterizationInfo.depthBiasClamp = 0.0f;           // Optional
        configInfo.rasterizationInfo.depthBiasSlopeFactor = 0.0f;     // Optional

        //multisampleing - edges of geometry(triangle)
        //how much of pixel is contained in triangle
